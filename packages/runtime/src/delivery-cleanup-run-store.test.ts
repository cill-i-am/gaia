...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "gaia-cleanup-events-"));
  roots.push(root);
  ...
  const events = [
    ...
    ...receipts.map((receipt, index) =>
      event(index + 3, "DELIVERY_MERGE_RECORDED", {
        mergeAction: encodeDeliveryMergeReceiptJson(receipt),
      })
    ),
  ];
  writeFileSync(
  ...
  writeFileSync(
    ...
    JSON.stringify({
      repositoryCommonDir: path.join(root, ".git"),
      repositoryRoot: root,
      token: "private-token",
      version: 1,
      workspaceCommonDir: path.join(root, ".git"),
...
describe("delivery cleanup authoritative run-store integration", () => {
  it("persists provenance before inspection and resumes only the remaining exact resource", async () => {
    ...
    const first: CleanupResourceAdapter = {
      inspectWorktree: () =>
        Effect.sync(() => {
          calls.push("first:inspectWorktree");
          return "absent" as const;
        }),
      removeWorktree: () => Effect.die("must not remove absent worktree"),
      inspectBranch: () =>
        Effect.sync(() => {
          calls.push("first:inspectBranch");
          return branch;
        }),
      removeBranchCas: () =>
        Effect.sync(() => {
          calls.push("first:removeBranchCas");
    ...
    await expect(
      run(
        coordinateDeliveryCleanup(
          ...
          { cleanupResourceAdapter: first, rootDirectory: f.root }
        )
      )
    ).rejects.toBeTruthy();
    const afterFailure = eventTypes(f.paths.events);
    expect(
      afterFailure.findIndex(
        ({ type }) => type === "DELIVERY_CLEANUP_PROVENANCE_RECORDED"
      )
    ).toBeLessThan(
      afterFailure.findIndex(
    ...
    const restarted = makeRecordingCleanupResourceAdapter({
      branch: "present",
      worktree: "absent",
    });
    const completed = await run(
      coordinateDeliveryCleanup(
        ...
        { cleanupResourceAdapter: restarted.adapter, rootDirectory: f.root }
      )
    );
    ...
    const replay = snapshotFromReplay(
      eventTypes(f.paths.events).map((value) =>
        Schema.decodeUnknownSync(RunEvent)(value)
      )
    );
  ...
  it("re-proves a completed same action idempotently and rejects a conflicting action", async () => {
    const f = fixture();
    const first = makeRecordingCleanupResourceAdapter({
      branch: "absent",
      worktree: "absent",
    });
    const action = {
    ...
    await run(
      coordinateDeliveryCleanup(f.runId, action, {
        cleanupResourceAdapter: first.adapter,
    ...
    await run(
      coordinateDeliveryCleanup(f.runId, action, {
        cleanupResourceAdapter: restarted.adapter,
        rootDirectory: f.root,
      })
    );
