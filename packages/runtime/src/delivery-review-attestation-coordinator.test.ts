...
function fixture() {
  ...
  const attempt = DeliveryPublicationAttempted.make({
    ...publicationBase,
  ...
  const events = [
    ...
    event(4, "DELIVERY_PUBLICATION_ATTEMPTED", {
      publication: encodeDeliveryPublicationJson(attempt),
    }),
  ...
  const fresh: FreshReadyForReviewState = {
...
function attestationReceipts(f: ReturnType<typeof fixture>) {
  return readFileSync(f.paths.events, "utf8")
    ...
    .map(({ payload }) =>
      parseDeliveryLocalReviewAttestationReceipt(payload["attestation"])
    );
}
...
async function ready(f: ReturnType<typeof fixture>) {
  return Effect.runPromise(
    coordinateDeliveryPullRequestReady(
      ...
      { freshStateReader: () => Effect.succeed(f.fresh), rootDirectory: f.root }
...
describe("local operator paired-review attestation coordinator", () => {
  it("records intent then confirms an exact open current-head action without provider mutation", async () => {
    ...
    expect(result).toMatchObject({
      ...
      decision: "approved",
      headSha: f.publication.headSha,
      state: "confirmed",
    });
    expect(result.gaiaEvidenceId).toMatch(/^evidence-[A-Za-z0-9_-]{16,120}$/u);
    expect(reads).toBe(1);
    expect(attestationReceipts(f).map(({ state }) => state)).toEqual([
      "intentRecorded",
      "confirmed",
    ]);
  ...
  it("keeps intent active when the mandatory fresh read is unavailable and resumes the same ID", async () => {
    ...
    await expect(
      Effect.runPromise(
        coordinateDeliveryLocalReviewAttestation(f.runId, action, {
          freshStateReader: () => Effect.fail("offline"),
          rootDirectory: f.root,
        }).pipe(Effect.provide(NodeServices.layer))
      )
    ).rejects.toMatchObject({ code: "DeliveryReviewAttestationReadFailed" });
    expect(attestationReceipts(f).map(({ state }) => state)).toEqual([
      "intentRecorded",
    ]);
    ...
    const result = await Effect.runPromise(
      coordinateDeliveryLocalReviewAttestation(f.runId, action, {
        freshStateReader: () => Effect.succeed(f.fresh),
        rootDirectory: f.root,
      }).pipe(Effect.provide(NodeServices.layer))
    );
    expect(result.state).toBe("confirmed");
    expect(attestationReceipts(f)[0]?.gaiaEvidenceId).toBe(
      result.gaiaEvidenceId
    );
  });
  ...
  it("admits one concurrent duplicate and lets the lock-conflicted request retry idempotently", async () => {
    ...
    let reads = 0;
    const action = {
      actionId: "attestation-concurrent-1",
      decision: "approved" as const,
      expectedBranchName: f.publication.branchName,
      expectedHeadSha: f.publication.headSha,
      expectedPrNumber: f.publication.prNumber,
    ...
    const fulfilled = concurrent.filter(
      (
        result
      ): result is PromiseFulfilledResult<Awaited<ReturnType<typeof invoke>>> =>
  ...
  it.each([
    ...
    [
      "merged",
      {
        ...
        mergedAt: "2026-07-13T10:10:00.000Z",
        mergeCommitSha: "b".repeat(40),
      },
    ],
  ])(
    "records terminal failure when the exact pull request is %s",
    async (_name, override) => {
      ...
      const result = await Effect.runPromise(
        coordinateDeliveryLocalReviewAttestation(
          ...
          {
            freshStateReader: () => Effect.succeed({ ...f.fresh, ...override }),
            rootDirectory: f.root,
          }
        ).pipe(Effect.provide(NodeServices.layer))
      );
