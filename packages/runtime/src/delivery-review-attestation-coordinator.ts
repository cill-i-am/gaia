...
export type DeliveryLocalReviewAttestationCoordinatorOptions =
  RunStorageOptions &
    Pick<
  ...
  return withRunStoreLock(
    ...
    Effect.gen(function* () {
      ...
      const events = yield* readEvents(paths).pipe(
        Effect.catchCause(() =>
          conflict(
            "Local review attestation history is invalid for the current run authority."
          )
        )
      );
      ...
      if (replay.state !== "delivering")
        return yield* conflict(
          "Local review attestation requires a delivering pull-request run."
        );
      ...
      if (publication.state !== "confirmed")
        return yield* conflict(
          "Local review attestation requires a confirmed owned pull request."
        );
      ...
      if (
        ...
        return yield* conflict(
          "Local review attestation does not match the authoritative current pull-request head."
        );
      ...
      const readyConfirmation = events.findLast((event) => {
        ...
        const candidate = parseDeliveryPullRequestReadyReceipt(
          event.payload["readyForReviewAction"]
        );
        return (
          candidate.actionId === ready.actionId &&
          candidate.payloadDigest === ready.payloadDigest &&
          (candidate.state === "confirmedWithoutDispatch" ||
            candidate.state === "dispatchConfirmed")
        );
      });
      if (readyConfirmation === undefined)
        return yield* conflict(
          "Exact ready-for-review confirmation evidence is unavailable."
        );

      const gaiaEvidenceId = deterministicEvidenceId({
        actionId: action.actionId,
        authoritySequence: authority.authoritySequence,
        headSha: authority.headSha,
        publicationPayloadDigest: publication.payloadDigest,
        readyPayloadDigest: ready.payloadDigest,
        runId,
      });
      const bindingBase = {
        actionId: action.actionId,
        ...
        ...(action.gaiaEvidenceDigest === undefined
          ? {}
          : { gaiaEvidenceDigest: action.gaiaEvidenceDigest }),
        gaiaEvidenceId,
        headSha: authority.headSha,
        prNumber: publication.prNumber,
        prUrl: publication.prUrl,
      ...
      const binding = {
        ...
        attestationPayloadDigest:
          deliveryLocalReviewAttestationPayloadDigest(bindingBase),
      };
      const histories =
      ...
      if (
        previous !== undefined &&
        previous.attestationPayloadDigest !== binding.attestationPayloadDigest
      ) {
        return yield* conflict(
          "Local review attestation action ID conflicts with a changed immutable tuple."
        );
      }
      if (
      ...
      const currentConfirmed = histories.histories.find(
        ({ latest }) =>
      ...
      if (
        ...
        currentConfirmed.actionId !== action.actionId
      ) {
        return yield* conflict(
          "The current delivery head already has a confirmed local review attestation."
        );
      }
      if (previous?.state === "confirmed" || previous?.state === "failed")
        return previous;

      if (previous === undefined) {
        yield* appendAttestation(
          runId,
          paths,
      ...
      const fresh = yield* reader({
        ...
        Effect.mapError(() =>
          makeRuntimeError({
            code: "DeliveryReviewAttestationReadFailed",
            message:
              "Fresh pull-request lifecycle state is unavailable; the attestation intent remains active.",
            recoverable: true,
          })
        )
      );
      if (!isExactOpenPullRequest(fresh, binding)) {
        const failed = DeliveryLocalReviewAttestationFailed.make({
          ...binding,
          code: "DeliveryReviewAttestationPullRequestUnavailable",
          message:
            "Fresh state does not prove the exact pull request is open, unmerged, and non-draft.",
          state: "failed",
        });
      ...
      const latestDelivery = Schema.decodeUnknownSync(
        Schema.Record(Schema.String, Schema.Json)
      )(latestReplay.context["delivery"]);
      const latestPublication = parseDeliveryPublication(
        latestDelivery["publication"]
      );
      if (latestPublication.state !== "confirmed")
        return yield* conflict(
          "Confirmed publication changed before local review attestation confirmation."
        );
      const latestAuthority = deriveDeliveryAuthority(
        latestPublication,
        latestEvents
      );
      if (
        latestPublication.operationId !== publication.operationId ||
        latestPublication.payloadDigest !== publication.payloadDigest ||
        latestAuthority.authoritySequence !== authority.authoritySequence ||
        latestAuthority.headSha !== authority.headSha
      ) {
        return yield* conflict(
          "Delivery authority changed before local review attestation confirmation."
        );
      }
    ...
    }),
    {
      ...
      nextSafeAction:
        "Refresh delivery state and resume only the same exact attestation action ID.",
    }
  );
}
...
function appendAttestation(
  ...
  return appendEvent(runId, paths, {
    payload: {
      attestation: encodeDeliveryLocalReviewAttestationReceiptJson(receipt),
    },
...
function deterministicEvidenceId(input: {
  ...
  const digest = createHash("sha256")
    .update(
      [
        "gaia.delivery.local-paired-review-evidence-id.v1",
        input.actionId,
        input.runId,
        input.publicationPayloadDigest,
        String(input.authoritySequence),
        input.headSha,
        input.readyPayloadDigest,
      ].join("\0")
    )
    .digest("hex");
