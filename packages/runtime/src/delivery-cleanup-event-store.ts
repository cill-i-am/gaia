...
import type {
  ...
  CleanupResourceCheckpoint,
} from "./delivery-cleanup-resource-coordinator.js";
import { makeRuntimeError } from "./errors.js";
import { appendEvent, loadRun } from "./event-store.js";
import type { RunPaths } from "./paths.js";

export function recordOrValidateCleanupProvenance(
  runId: RunId,
  paths: RunPaths,
  expected: DeliveryCleanupOwnershipProvenanceV1
) {
  return Effect.gen(function* () {
    ...
    const recorded = loaded.events
      ...
      .map((event) =>
        parseDeliveryCleanupOwnershipProvenance(event.payload["provenance"])
      );
    if (recorded.length === 0) {
      yield* appendEvent(runId, paths, {
        payload: {
          provenance: encodeDeliveryCleanupOwnershipProvenanceJson(expected),
        },
    ...
    if (
      match === undefined ||
      match.payloadDigest !== expected.payloadDigest ||
      JSON.stringify(match) !== JSON.stringify(expected)
    ) {
      return yield* Effect.fail(
        makeRuntimeError({
          code: "DeliveryActionConflict",
          message: "Cleanup action conflicts with durable private provenance.",
          recoverable: true,
        })
      );
    }
    if (
      ...
      return yield* Effect.fail(
        makeRuntimeError({
          code: "DeliveryActionConflict",
          message: "Another cleanup action is already authoritative.",
          recoverable: true,
        })
      );
    }
...
export function makeEventCleanupCheckpointStore(
  runId: RunId,
  paths: RunPaths
): CleanupCheckpointStore {
  return {
    append: (checkpoint) =>
      appendEvent(runId, paths, {
        payload: {
          checkpoint: encodeDeliveryCleanupResourceCheckpointJson(
            DeliveryCleanupResourceCheckpointV1.make({
              ...checkpoint,
              version: 1,
            })
          ),
      ...
      loadRun(paths).pipe(
        Effect.map(({ events }) =>
          events.flatMap((event) =>
            event.type === "DELIVERY_CLEANUP_RESOURCE_CHECKPOINT_RECORDED"
              ? [
                  parseDeliveryCleanupResourceCheckpoint(
                    event.payload["checkpoint"]
                  ) satisfies CleanupResourceCheckpoint,
                ]
              : []
          )
        ),
