...
export interface CleanupResourceAdapter {
  readonly inspectWorktree: (provenance: DeliveryCleanupOwnershipProvenanceV1) => Effect.Effect<CleanupResourceInspection, unknown>;
  readonly removeWorktree: (provenance: DeliveryCleanupOwnershipProvenanceV1) => Effect.Effect<void, unknown>;
  readonly inspectBranch: (provenance: DeliveryCleanupOwnershipProvenanceV1) => Effect.Effect<CleanupResourceInspection, unknown>;
  readonly removeBranchCas: (provenance: DeliveryCleanupOwnershipProvenanceV1) => Effect.Effect<void, unknown>;
}
...
export type CleanupResourceCheckpoint = {
  ...
  readonly state: "absenceProven" | "inspectedAbsent" | "inspectedPresent" | "removalAttempted";
};
...
export type CleanupBranchBoundary = "afterBranchAbsentCheckpoint" | "afterBranchInspection" | "afterBranchInspectionCheckpoint" | "afterBranchPostInspection" | "afterBranchRemoval" | "afterBranchRemovalCheckpoint";

export class CleanupCrashInjected extends Data.TaggedError("CleanupCrashInjected")<{
  readonly boundary: CleanupWorktreeBoundary | CleanupBranchBoundary;
}> {}

export type CleanupCheckpointStore = {
  readonly append: (checkpoint: CleanupResourceCheckpoint) => Effect.Effect<void, unknown>;
  readonly read: () => Effect.Effect<ReadonlyArray<CleanupResourceCheckpoint>, unknown>;
};
...
export function coordinateWorktreeCleanup(input: {
  ...
  const checkpoint = (state: CleanupResourceCheckpoint["state"]) => input.checkpointStore.append({
  ...
  return Effect.gen(function* () {
    ...
    if (history.some(({ resource, state }) => resource === "worktree" && state === "absenceProven")) {
      const fresh = yield* input.adapter.inspectWorktree(input.provenance);
      if (fresh !== "absent") return yield* Effect.fail(new Error("Previously absent owned worktree reappeared."));
    ...
    yield* checkpoint(initial === "present" ? "inspectedPresent" : "inspectedAbsent");
    ...
    yield* checkpoint(after === "absent" ? "inspectedAbsent" : "inspectedPresent");
...
export function coordinateBranchCleanup(input: { readonly adapter: CleanupResourceAdapter; readonly checkpointStore: CleanupCheckpointStore; readonly crashAfter?: CleanupBranchBoundary; readonly provenance: DeliveryCleanupOwnershipProvenanceV1 }) {
  const checkpoint = (state: CleanupResourceCheckpoint["state"]) => input.checkpointStore.append({ actionId: input.provenance.actionId, payloadDigest: input.provenance.payloadDigest, resource: "branch", state });
  const crash = (boundary: CleanupBranchBoundary) => input.crashAfter === boundary ? Effect.fail(new CleanupCrashInjected({ boundary })) : Effect.void;
  return Effect.gen(function* () {
    const history = yield* input.checkpointStore.read();
    assertCheckpointBinding(history, input.provenance);
    if (history.some(({ resource, state }) => resource === "branch" && state === "absenceProven")) {
      const fresh = yield* input.adapter.inspectBranch(input.provenance);
      if (fresh !== "absent") return yield* Effect.fail(new Error("Previously absent owned branch reappeared."));
      yield* checkpoint("inspectedAbsent");
      yield* checkpoint("absenceProven");
      return "absent" as const;
    }
...
function assertCheckpointBinding(
  ...
  if (history.some((checkpoint) => checkpoint.actionId !== provenance.actionId || checkpoint.payloadDigest !== provenance.payloadDigest)) {
    throw new Error("Cleanup checkpoint binding changed.");
  }
}

export function makeRecordingCleanupResourceAdapter(initial: {
  ...
  const adapter: CleanupResourceAdapter = {
    inspectBranch: () => Effect.sync(() => { calls.push("inspectBranch"); return branch; }),
    inspectWorktree: () => Effect.sync(() => { calls.push("inspectWorktree"); return worktree; }),
    removeBranchCas: () => Effect.sync(() => { calls.push("removeBranchCas"); branch = "absent"; }),
    removeWorktree: () => Effect.sync(() => { calls.push("removeWorktree"); worktree = "absent"; }),
  };
  return { adapter, calls, state: () => ({ branch, worktree }) };
}

export function makeInMemoryCleanupCheckpointStore(seed: ReadonlyArray<CleanupResourceCheckpoint> = []) {
  const checkpoints = [...seed];
  const store: CleanupCheckpointStore = {
    append: (checkpoint) => Effect.sync(() => { checkpoints.push(checkpoint); }),
