...
describe("delivery acceptance provenance policy", () => {
  ...
  it("resolves an explicit literal remote base and head with exact argv", async () => {
    ...
    expect(commands.map(({ args }) => args)).toEqual([
      ...
      [
        ...
        "--no-tags",
        policy.remote,
        `refs/heads/${policy.baseBranch}:refs/remotes/${policy.remote}/${policy.baseBranch}`,
      ],
      [
        "rev-parse",
        "--verify",
        `refs/remotes/${policy.remote}/${policy.baseBranch}^{commit}`,
      ],
    ]);
  });
  ...
  it("preserves the exact default resolver behavior", async () => {
    const commands: GitDeliveryCommandInput[] = [];
    const result = await Effect.runPromise(
      resolveDeliveryProvenance("run-1234567890", {
        commandRunner: recording(commands),
        rootDirectory: root,
      })
    );
  ...
  for (const input of [
    ...
    {
      baseBranch: "main",
      headBranch: "refs/heads/injected",
      remote: "origin",
      version: 1,
    },
    {
    ...
    },
    {
      baseBranch: "main",
      headBranch: "gaia/head",
      remote: "git@github.com:cill-i-am/gaia",
      version: 1,
    },
    {
      baseBranch: "main",
    ...
    it(`rejects hostile acceptance policy ${JSON.stringify(input)}`, async () => {
      ...
      await expect(
        Effect.runPromise(
          resolveDeliveryProvenance(
            "run-1234567890",
            { commandRunner: recording(commands), rootDirectory: root },
            input
          )
        )
      ).rejects.toBeTruthy();
  ...
  it("rejects a same-branch topology before ref validation or fetch", async () => {
    ...
    ).rejects.toMatchObject({ code: "DeliveryProvenanceTopologyInvalid" });
    expect(commands).toEqual([]);
  });
});
  ...
  return (command: GitDeliveryCommandInput) =>
    Effect.sync(() => {
      ...
      if (
        ...
        (command.args[1] === "origin/main" || command.args[1] === "--verify")
      )
        return { stderr: "", stdout: `${oid}\n` };
      return {
        stderr: "",
        stdout:
