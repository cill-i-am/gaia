...
describe("delivery remediation activation envelope", () => {
  it.effect("persists one immutable private envelope and removes only an exact verified receipt", () =>
    Effect.gen(function* () {
      const root = yield* Effect.promise(() => import("node:fs/promises").then(({ mkdtemp }) => mkdtemp("/tmp/gaia-activation-")));
      const request = activationRequest();
      const envelope = makeEnvelope(request);
      const changedAction = makeEnvelope(DeliveryRemediationActivationActionRequest.make({
        ...request,
      ...
      const loaded = yield* store.load(envelope.runId, envelope.authorization.authorizationDigest);
      ...
      assert.strictEqual(loaded?.activationReceiptDigest, envelope.activationReceiptDigest);
      assert.notStrictEqual(changedAction.activationReceiptDigest, envelope.activationReceiptDigest);
      assert.strictEqual((yield* Effect.promise(() => stat(path.dirname(target)))).mode & 0o777, 0o700);
      assert.strictEqual((yield* Effect.promise(() => stat(target))).mode & 0o777, 0o600);
      assert.notInclude(yield* Effect.promise(() => readFile(target, "utf8")), "/private/tmp");

      assert.isFalse(yield* store.removeVerified({
        authorizationDigest: envelope.authorization.authorizationDigest,
        receiptDigest: "f".repeat(64),
        runId: envelope.runId,
      }));
      assert.isTrue(yield* store.removeVerified({
        authorizationDigest: envelope.authorization.authorizationDigest,
        receiptDigest: envelope.activationReceiptDigest,
        runId: envelope.runId,
      }));
      assert.isFalse(yield* Effect.promise(() => lstat(target).then(() => true, () => false)));
    }),
  );
  ...
  it.effect("rejects symlinked and corrupt activation files", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => mkdir(path.dirname(target), { mode: 0o700, recursive: true }));
      yield* Effect.promise(() => writeFile(`${target}.source`, "{}", { mode: 0o600 }));
      yield* Effect.promise(() => symlink(`${target}.source`, target));
      assert.isTrue((yield* Effect.exit(store.load(envelope.runId, envelope.authorization.authorizationDigest)))._tag === "Failure");

      yield* Effect.promise(() => import("node:fs/promises").then(({ unlink }) => unlink(target)));
      yield* Effect.promise(() => writeFile(target, "{\"version\":1}", { mode: 0o600 }));
      assert.isTrue((yield* Effect.exit(store.load(envelope.runId, envelope.authorization.authorizationDigest)))._tag === "Failure");
  ...
  it.effect("rejects a symlink anywhere below the accepted run root", () =>
    Effect.gen(function* () {
      const root = yield* Effect.promise(() => import("node:fs/promises").then(({ mkdtemp }) => mkdtemp("/tmp/gaia-activation-")));
      const outside = yield* Effect.promise(() => import("node:fs/promises").then(({ mkdtemp }) => mkdtemp("/tmp/gaia-activation-outside-")));
      const envelope = makeEnvelope(activationRequest());
      const store = makeFileDeliveryRemediationActivationStore(root);
      yield* Effect.promise(() => symlink(outside, path.join(root, ".gaia")));

      assert.strictEqual((yield* Effect.exit(store.save(envelope)))._tag, "Failure");
      assert.strictEqual((yield* Effect.exit(store.load(
        envelope.runId,
        envelope.authorization.authorizationDigest,
      )))._tag, "Failure");
    }),
