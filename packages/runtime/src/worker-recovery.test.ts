...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "gaia-worker-recovery-"));
  roots.push(root);
  ...
  const events = [
    event(1, "RUN_CREATED", { specPath: "input.md" }),
  ...
  events[0] = event(1, "RUN_CREATED", {
    specPath: "input.md",
    delivery: { baseRevision: "a".repeat(40), mode: "pullRequest" },
...
async function realOwnedFixture() {
  const f = fixture();
  rmSync(f.paths.workspace, { recursive: true, force: true });
  git(f.root, "init", "-b", "main");
  git(f.root, "config", "user.email", "gaia@test.invalid");
  git(f.root, "config", "user.name", "Gaia Test");
  writeFileSync(path.join(f.root, "README.md"), "base\n");
  writeFileSync(
    path.join(f.root, ".gitignore"),
    [".gaia/", ".turbo/", "dist/", "**/.gaia/", "**/.turbo/", "**/dist/"].join(
      "\n"
    ) + "\n"
  );
  git(f.root, "add", "README.md", ".gitignore");
  git(f.root, "commit", "-m", "base");
  const remote = mkdtempSync(
    path.join(tmpdir(), "gaia-worker-recovery-remote-")
  );
  roots.push(remote);
  ...
  created.payload.delivery = provenance;
  ...
  const validateWorkspace = () =>
    Effect.gen(function* () {
      const latest = JSON.parse(
        readFileSync(f.paths.events, "utf8").split("\n")[0]!
      );
      const accepted = parseDeliveryProvenance(latest.payload.delivery);
      if (accepted._tag === "None")
        return yield* Effect.fail(new Error("invalid provenance"));
      yield* inspectRecoverableDeliveryWorktreeOwnership({
        expectedHeads: [head],
        options: { rootDirectory: f.root },
        paths: f.paths,
...
describe("recoverWorkerSession", () => {
  it("partitions worker recovery authority by exact failure generation", async () => {
    ...
    const secondFailure = await run(
      appendEvent(f.runId, f.paths, {
        payload: {
          code: "HarnessSessionFailed",
    ...
    expect(replayOld).toMatchObject({
      ...
      nativeTurnIdDigest:
        first.state === "dispatchConfirmed"
          ? first.nativeTurnIdDigest
          : undefined,
      state: first.state,
    });
    expect(starts).toBe(1);
    ...
    expect(replaySecond).toMatchObject({
      ...
      nativeTurnIdDigest:
        second.state === "dispatchConfirmed"
  ...
  it("rejects stale private checkpoints from older recovery generations", async () => {
    ...
    if (first.state !== "dispatchConfirmed")
      throw new Error("expected confirmed recovery");
  ...
  it("serializes concurrent same-generation recovery actions to one dispatch", async () => {
    ...
    if (fulfilled === undefined || fulfilled.status !== "fulfilled")
      throw new Error("expected one fulfilled recovery");
  ...
  it("allows dirty registered owned worktrees for continuation ownership", async () => {
    ...
    writeFileSync(
      path.join(f.paths.workspace, "payload-change.txt"),
      "dirty payload\n"
    );
  ...
  it("fingerprints tracked payload while allowing ignored generated output churn", async () => {
    const f = await realOwnedFixture();
    writeFileSync(
      path.join(f.paths.workspace, "README.md"),
      "payload change\n"
    );
    mkdirSync(path.join(f.paths.workspace, "apps/server/.gaia"), {
      recursive: true,
    });
    mkdirSync(path.join(f.paths.workspace, "packages/runtime/dist"), {
      recursive: true,
    });
    mkdirSync(path.join(f.paths.workspace, ".turbo"), { recursive: true });
    writeFileSync(
      path.join(f.paths.workspace, "apps/server/.gaia/events.jsonl"),
      "generated\n"
    );
    writeFileSync(
      path.join(f.paths.workspace, "packages/runtime/dist/index.js"),
      "generated\n"
    );
    writeFileSync(
      path.join(f.paths.workspace, ".turbo/cache.bin"),
      "generated\n"
    );
  ...
  it.each([
    [
      "unexpected unignored untracked file",
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        mkdirSync(path.join(f.paths.workspace, "src"), { recursive: true });
        writeFileSync(
          path.join(f.paths.workspace, "src/new-file.ts"),
          "untracked\n"
        );
      },
    ],
    [
      ...
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        ...
        writeFileSync(
          path.join(f.paths.workspace, "dist/tracked.js"),
          "generated\n"
        );
    ...
    "rejects retained payload contamination: %s",
    async (_name, contaminate) => {
      const f = await realOwnedFixture();
      writeFileSync(
        path.join(f.paths.workspace, "README.md"),
        "payload change\n"
  ...
  it.each([
    [
      ...
      (
        ...
        manifest.workspaceRoot = `${f.paths.workspace}-other`;
      },
    ],
    [
      "common-dir",
      (
        _f: Awaited<ReturnType<typeof realOwnedFixture>>,
        manifest: Record<string, unknown>
      ) => {
        manifest.workspaceCommonDir = "/wrong/common";
      },
    ],
    [
      "base",
      (
        ...
        manifest.baseRevision = "b".repeat(40);
      },
    ],
    [
      "head",
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        writeFileSync(
          path.join(f.paths.workspace, "advanced.txt"),
          "advance\n"
        );
        git(f.paths.workspace, "add", "advanced.txt");
        git(f.paths.workspace, "commit", "-m", "advance");
      },
    ],
    [
      "primary-identity",
      (
        ...
        manifest.workspaceRoot = f.root;
      },
    ],
    [
      "registration",
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        git(f.root, "worktree", "remove", f.paths.workspace);
        mkdirSync(f.paths.workspace);
      },
    ],
  ] as const)(
    "rejects real continuation %s identity drift without requiring cleanliness",
    async (_name, mutate) => {
      const f = await realOwnedFixture();
      const manifest = JSON.parse(
        readFileSync(f.paths.deliveryOwnershipManifest, "utf8")
      );
      mutate(f, manifest);
      if (_name !== "registration" && _name !== "head") {
        writeFileSync(
          f.paths.deliveryOwnershipManifest,
  ...
  it.each([
    ...
    [
      ...
      (
        ...
        manifest.repositoryRoot = "/wrong/repository";
      },
    ],
    [
      "base",
    ...
    [
      ...
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        const lines = readFileSync(f.paths.events, "utf8").trim().split("\n");
        const created = JSON.parse(lines[0]!);
        created.payload.delivery.headBranch = "gaia/other";
        lines[0] = JSON.stringify(created);
        writeFileSync(f.paths.events, `${lines.join("\n")}\n`);
      },
    ],
    [
      "ownership-digest",
    ...
    [
      "cleanliness",
      (f: Awaited<ReturnType<typeof realOwnedFixture>>) => {
        writeFileSync(path.join(f.paths.workspace, "dirty.txt"), "dirty\n");
      },
    ],
    [
      "registration",
    ...
    "rejects real %s identity drift with zero events and zero provider calls",
    async (_name, mutate) => {
      const f = await realOwnedFixture();
      let calls = 0;
      const manifest = JSON.parse(
  ...
  it("runs the public server workflow A/B restart and non-mutation matrix against a real retained Git root", async () => {
    ...
    const beforeHead = git(f.paths.workspace, "rev-parse", "HEAD");
    const beforeStatus = git(f.paths.workspace, "status", "--porcelain");
    const beforeCommon = git(
      f.paths.workspace,
      "rev-parse",
    ...
    const confirmed = await run(
      actOnWorkerRecovery(f.runId, action, {
        rootDirectory: f.root,
        workerRecoveryActivator: activate,
      })
    );
    expect(confirmed.state).toBe("dispatchConfirmed");
    if (confirmed.state !== "dispatchConfirmed")
      throw new Error("expected confirmed recovery");
    ...
    expect(restarted).toMatchObject({
      nativeTurnIdDigest: confirmed.nativeTurnIdDigest,
      state: "dispatchConfirmed",
    });
    expect(starts).toBe(1);
    expect(git(f.paths.workspace, "rev-parse", "HEAD")).toBe(beforeHead);
    expect(git(f.paths.workspace, "status", "--porcelain")).toBe(beforeStatus);
    expect(
      git(
        f.paths.workspace,
        "rev-parse",
    ...
    expect(git(f.root, "worktree", "list", "--porcelain")).toBe(
      beforeInventory
    );
  });
  ...
  it("makes an ambiguous dispatch terminal and never redispatches", async () => {
    ...
    expect(second.state).toBe("outcomeUnknown");
    expect(starts).toBe(1);
  });

  it.each([
    [
      "missing sessionFailed",
      (events: Array<Record<string, unknown>>): void => {
        events[8] = {
          ...events[6]!,
          sequence: 9,
    ...
    [
      ...
      (events: Array<Record<string, unknown>>): void => {
        (
          (events[8]!["payload"] as Record<string, unknown>)["event"] as Record<
            string,
            unknown
          >
        )["sessionId"] = "session-run-other";
      },
      action,
    ],
    [
      "wrong harness profile",
      (events: Array<Record<string, unknown>>): void => {
        (
          (
        ...
        )["harnessProfileId"] = "otherProfile";
      },
      action,
    ],
    [
      "nonrecoverable provider failure",
      (events: Array<Record<string, unknown>>): void => {
        (
          (
            (events[8]!["payload"] as Record<string, unknown>)[
              "event"
            ] as Record<string, unknown>
    ...
    [
      "nonrecoverable run failure",
      (events: Array<Record<string, unknown>>): void => {
        (events[9]!["payload"] as Record<string, unknown>)["recoverable"] =
          false;
      },
      action,
    ],
    [
      "wrong failure stage",
      (events: Array<Record<string, unknown>>): void => {
        (events[9]!["payload"] as Record<string, unknown>)["stage"] =
          "reviewing";
      },
      action,
    ],
    [
      ...
      (_events: Array<Record<string, unknown>>): void => undefined,
      WorkerRecoveryAction.make({ ...action, expectedFailureSequence: 9 }),
    ],
    [
      "failure not final",
      (events: Array<Record<string, unknown>>): void => {
    ...
    "requires historical Gaia eligibility despite safe live status: %s",
    async (_name, mutate, request) => {
      const f = fixture();
  ...
  it.each([
    ...
    "records preflight failure for %s with zero dispatch",
    async (_name, scenario) => {
      ...
      if (result.state !== "failed")
        throw new Error("expected failed recovery");
      ...
      expect(
        existsSync(path.join(f.paths.root, ".worker-recovery-turn.json"))
      ).toBe(false);
    }
  );
    ...
    const exit = await run(
      recoverWorkerSession(f.runId, action, {
        appendRecoveryEvent: () =>
          Effect.fail(
            makeRuntimeError({
              cause: new Error("private persistence path"),
              code: "WriteFailed",
