...
describe("local run read index", () => {
  layer(NodeServices.layer)((it) => {
    it.effect(
      "rebuilds deterministically and refreshes runs without rescanning list",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const cwd = yield* fs.makeTempDirectory({
            prefix: "gaia-run-index-",
          });
          ...
          const directDetail = yield* readLocalRun(second.runId, {
            rootDirectory: cwd,
          });
    ...
    it.effect(
      "rebuilds diagnostics for bad run directories created after startup",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const cwd = yield* fs.makeTempDirectory({
    ...
    it.effect(
      "refreshes parseable bad-run diagnostics created after startup",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const cwd = yield* fs.makeTempDirectory({
    ...
    it.effect(
      "removes stale indexed runs when a targeted refresh finds them missing",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const cwd = yield* fs.makeTempDirectory({
          ...
          yield* fs.remove(`${store.runsRoot}/${summary.runId}`, {
            recursive: true,
          });
    ...
    it.effect(
      "refreshes terminal server workflow projections from events.jsonl",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const cwd = yield* fs.makeTempDirectory({
          ...
          assert.strictEqual(
            refreshedDetail.latestEventType,
            "REPORT_COMPLETED"
          );
          assert.deepEqual(refreshedDetail, directDetail);
        })
