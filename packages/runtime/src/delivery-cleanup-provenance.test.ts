...
describe("private delivery cleanup provenance", () => {
  it("binds every private ownership field into one immutable digest", () => {
    ...
    const parsed = parseDeliveryCleanupOwnershipProvenance({
      ...base,
      payloadDigest,
    });
  ...
  it("does not equate removal attempts with proven absence", () => {
    ...
    expect(
      deriveCleanupResourceProofs([checkpoint("worktree", "removalAttempted")])
    ).toEqual({ branch: false, worktree: false });
    expect(
      deriveCleanupResourceProofs([
        checkpoint("worktree", "absenceProven"),
        checkpoint("branch", "absenceProven"),
      ])
    ).toEqual({ branch: true, worktree: true });
  });
});
