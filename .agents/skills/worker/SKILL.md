...
2. Use `worktree-isolation` to fetch/prune the remote, require a valid symbolic
   `origin/HEAD`, prove the isolated tree begins clean at the exact dispatched
   SHA, and create the worker-owned `codex/<issue>-<slug>` branch there. Record
   the fetch time and exact base in the durable dispatch evidence.
...
For an explicit resume or special-ref instruction, preserve the named history
and prove the requested ref, exact head, fetched remote default, merge-base,
ahead/behind counts, and clean/dirty state. Do not reset, clean, merge,
auto-rebase, force-move, or discard work to manufacture fresh-base equality.

When work crosses an Alchemy or provider boundary, keep the provider effect at
the authorized stage: build and prove separable local artifacts first, then
stop at any unauthorized deploy, publication, credential, spend, production,
...
Bounded subagents may help with independent research or narrow evidence when
the active task explicitly permits them, but the worker remains owner of the
issue, branch, integration, proof, and handoff.
...
Create follow-ups only for concrete out-of-scope work, link them to the source
issue or parent outcome, and leave prioritization to the orchestrator.
