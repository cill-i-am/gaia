...
base_sha=$(git rev-parse --verify "${remote_head}^{commit}") || exit 1
...
3. Fetch and resolve the exact base once for the lane being created. Record the
   fetch time and exact base in the durable dispatch evidence.
...
When Codex task tooling accepts a starting branch, pass `startingState: {
type: "branch", branchName: "origin/<default>" }`, then independently verify
the task worktree rather than trusting the request alone.
...
## Explicit Resume Or Special Ref

A durable instruction to resume an existing branch or use a special ref is an
exception to fresh-default equality, not permission to rewrite history. Fetch
first and prove the named ref, exact `HEAD`, fetched remote default,
merge-base, ahead/behind counts, and clean/dirty state. Preserve useful commits
and changes; never reset, clean, merge, auto-rebase, force-move, or discard them
to make the lane look fresh.
