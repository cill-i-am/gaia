...
Use the recorded fetch time and durable dispatch evidence; treat an explicit
resume or special-ref instruction as an exception only when that context names
...
- new worker lane based on local or stale state rather than the exact fetched
  remote default;
- missing or invalid symbolic `origin/HEAD`, unresolved commit or merge-base,
  or a task `startingState` that was not independently verified;
- resumed or special-ref work without the named override, exact head,
  fetched-default relationship, ahead/behind counts, and clean/dirty state;
