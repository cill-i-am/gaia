...
- When a `yt-dlp` extractor failure indicates an outdated dependency, report
  the installed version and update requirement. Require explicit user approval
  before updating a user-managed installation, then retry once only after an
  approved update.
