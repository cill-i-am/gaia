...
| Versioned run contract and honest proof-result vocabulary | implemented | [`packages/core/src/run-contract.ts`](../packages/core/src/run-contract.ts), [`packages/core/src/run-contract.test.ts`](../packages/core/src/run-contract.test.ts), [`packages/runtime/src/run-contract.ts`](../packages/runtime/src/run-contract.ts), [`packages/runtime/src/server-workflows.test.ts`](../packages/runtime/src/server-workflows.test.ts) | Source-authored outcomes, claims, non-goals, and stop conditions are recorded before execution. Missing proof stays non-passing, the framework marker is supplemental protocol evidence, and legacy verification replays as no-contract `completed-unverified`. |
| Accepted-outcome mapping and scoped claim execution | implemented | [`packages/core/src/run-contract-v2.ts`](../packages/core/src/run-contract-v2.ts), [`packages/core/src/claim-verification-replay.test.ts`](../packages/core/src/claim-verification-replay.test.ts), [`packages/runtime/src/verifier.ts`](../packages/runtime/src/verifier.ts), [`packages/runtime/src/docker-sandbox-verification-executor.test.ts`](../packages/runtime/src/docker-sandbox-verification-executor.test.ts) | Only specs with an exact source-owned `verification.version: 2` mapping can execute claims. The sole live command adapter is the pinned Docker Sandbox profile with denied network, no command credentials, no host fallback or retry, exact name/UUID cleanup, and typed unknown-outcome reconciliation. V1 remains permanently replayable and non-executing. |
...
| Durable run-wide wait, pause, resume, and cancel | implemented | [`packages/core/src/run-control.ts`](../packages/core/src/run-control.ts), [`packages/core/src/run-control.test.ts`](../packages/core/src/run-control.test.ts), [`packages/runtime/src/run-control-runtime.ts`](../packages/runtime/src/run-control-runtime.ts), [`packages/runtime/src/run-control-runtime.test.ts`](../packages/runtime/src/run-control-runtime.test.ts), [`apps/cli/src/main.ts`](../apps/cli/src/main.ts), [`apps/cli/src/main.test.ts`](../apps/cli/src/main.test.ts) | Run control is event-owned and authority-bound; session interaction is separate, and `outcomeUnknown` never permits blind redispatch. |
| Fixed-worker harness evaluation | partial | [`packages/core/src/harness-evaluation.ts`](../packages/core/src/harness-evaluation.ts), [`packages/core/src/harness-evaluation.test.ts`](../packages/core/src/harness-evaluation.test.ts), [`packages/runtime/src/harness-evaluation.ts`](../packages/runtime/src/harness-evaluation.ts), [`packages/runtime/src/harness-evaluation.test.ts`](../packages/runtime/src/harness-evaluation.test.ts) | Event-owned baseline commitments, fail-closed comparisons, provenance-separated metrics, rebuildable reads, and deterministic conformance scenarios exist. No fresh real-worker baseline or treatment has been run; architecture, availability, and lesson observations do not prove use or causal improvement. The qualitative factory scorecard remains separate. |
...
- GAIA-144's durable event vocabulary is forward-incompatible with older
  readers. Every reader and writer must roll forward before a new
  `RUN_CONTRACT_RECORDED` or `RUN_PROOF_RESULT_RECORDED` event is appended;
  mixed-version fleets and downgrade after those writes are unsupported.
- After the first V2 claim-verification event is written, every reader must
  retain the V2 contract, result, lifecycle, and projection codecs. Disabling
  new V2 execution must not downgrade or rewrite existing history.
