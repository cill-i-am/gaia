...
function makeFixture(
  ...
  return Effect.gen(function* () {
    ...
    const acceptanceRegistry =
      failureRegistry(mode ?? "missing checkpoint") ??
      makeTestHarnessProviderRegistry();
    ...
    const provenance = parseDeliveryProvenance(
      initial.events[0]?.payload["delivery"]
    );
    ...
    const firstFailureSequence = (yield* loadRun(paths)).events.at(
      -1
    )!.sequence;
    ...
    if (options.retainedDirty === true) {
      ...
      const secondFailure = yield* appendEvent(accepted.runId, paths, {
        payload: {
          code: "HarnessSessionFailed",
          message: "second failed",
          recoverable: true,
      ...
      yield* fs.writeFileString(
        ...
        "retained payload change\n"
      );
      yield* fs.makeDirectory(`${paths.workspace}/apps/server/.gaia`, {
        recursive: true,
      });
      yield* fs.makeDirectory(`${paths.workspace}/packages/runtime/dist`, {
        recursive: true,
      });
      yield* fs.makeDirectory(`${paths.workspace}/.turbo`, { recursive: true });
      yield* fs.writeFileString(
        `${paths.workspace}/apps/server/.gaia/events.jsonl`,
        "generated\n"
      );
      yield* fs.writeFileString(
        `${paths.workspace}/packages/runtime/dist/index.js`,
        "generated\n"
      );
      yield* fs.writeFileString(
        `${paths.workspace}/.turbo/cache.bin`,
        "generated\n"
      );
    }
    ...
    const activator = (
      ...
      Effect.gen(function* () {
        const receipt = yield* recoverWorkerSession(runId, request, {
          ...
          validateWorkspace: () =>
            options.productionWorkspaceValidation === true
              ? validateProductionWorkerRecoveryWorkspace({
                  action: request,
                  expectedHead: provenance.value.baseRevision,
                  rootDirectory: rootB,
                  runId,
                })
              : inspectRecoverableDeliveryWorktreeOwnership({
        ...
        if (
          ...
          if (mode === "corrupt checkpoint")
            yield* fs.writeFileString(checkpoint, "not-json");
          if (mode === "exact-turn mismatch")
            yield* fs.writeFileString(
              checkpoint,
              JSON.stringify({ turnId: "wrong-turn", version: 1 })
            );
        }
        return receipt;
      });
    const registry =
      mode === undefined
        ? makeTestHarnessProviderRegistry()
        : failureRegistry(mode);
    const workflowOptions =
      registry === undefined
        ? { workerRecoveryActivator: activator }
        : {
            harnessProviderRegistry: registry,
            workerRecoveryActivator: activator,
          };
    const server = makeLocalGaiaServerLayer(
      {
        host: "127.0.0.1",
        pid: process.pid,
        rootDirectory: rootB,
        serverId: "srv_http_recovery",
    ...
    const post = () =>
      HttpClientRequest.post(`/runs/${accepted.runId}/recovery/actions`).pipe(
        HttpClientRequest.bodyJsonUnsafe(action),
        HttpClient.execute,
        Effect.provide(server)
      );
    const getProjection = () =>
      HttpClient.get(`/runs/${accepted.runId}/delivery`).pipe(
        Effect.provide(server)
      );
    const cleanup = fs
      .remove(rootB, { recursive: true })
      .pipe(Effect.andThen(fs.remove(remote, { recursive: true })));
    return {
      action,
      before,
      cleanup,
...
describe("finite worker recovery HTTP lifecycle", () => {
  layer(NodeServices.layer)((it) => {
    it.effect(
      "continues retained Git root B to WORKER_COMPLETED and restarts without duplicate dispatch",
      () =>
        Effect.gen(function* () {
          ...
          assert.isTrue(
            after.events.some(({ type }) => type === "WORKER_COMPLETED")
          );
    ...
    it.effect(
      "allows later-generation retained dirty tracked payload through production recovery validation only",
      () =>
        Effect.gen(function* () {
          ...
          const receipts = events.events.flatMap((event) =>
            event.type === "WORKER_RECOVERY_RECORDED"
              ? [parseWorkerRecoveryReceipt(event.payload["recovery"])]
              : []
          );
          const confirmed = receipts.find(
            (receipt) =>
              receipt.actionId === production.action.actionId &&
              receipt.state === "dispatchConfirmed"
          );
          assert.strictEqual(confirmed?.state, "dispatchConfirmed");
          assert.match(
            confirmed?.trackedPayloadDigest ?? "",
            /^[a-f0-9]{64}$/u
          );
          assert.strictEqual(confirmed?.trackedPayloadEntryCount, 1);
          assert.notInclude(JSON.stringify(confirmed), production.rootB);
          assert.isTrue(
            events.events.some((event) => event.type === "WORKER_COMPLETED")
          );
          const continuationFailure = receipts.find(
            (receipt) =>
              receipt.actionId === production.action.actionId &&
              receipt.state === "failed" &&
              receipt.code === "WorkerRecoveryContinuationFailed"
          );
    ...
    for (const mode of [
      ...
      it.effect(
        `fails closed after confirmed dispatch on ${mode}`,
        () =>
          Effect.gen(function* () {
            const fixture = yield* makeFixture(mode);
            assert.strictEqual((yield* fixture.post()).status, 200);
            const first = yield* loadRun(fixture.paths);
            const receiptEvents = first.events.filter(
              ({ type }) => type === "WORKER_RECOVERY_RECORDED"
            );
            const receipt = parseWorkerRecoveryReceipt(
              receiptEvents.at(-1)?.payload["recovery"]
            );
            if (receipt.state !== "failed")
              throw new Error(`expected failed receipt, got ${receipt.state}`);
            ...
            const confirmedSequence = receiptEvents.find(
              ({ payload }) =>
                parseWorkerRecoveryReceipt(payload["recovery"]).state ===
                "dispatchConfirmed"
            )!.sequence;
            assert.lengthOf(
              first.events.filter(
            ...
            assert.isFalse(
              first.events.some(({ type }) => type === "WORKER_COMPLETED")
            );
            ...
            assert.strictEqual(fixture.starts(), 1);
            assert.isFalse(
              repeated.events.some(({ type }) => type === "WORKER_COMPLETED")
            );
            assert.deepEqual(inventory(fixture.rootB), fixture.before);
            yield* fixture.cleanup;
          }),
