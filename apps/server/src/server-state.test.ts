...
describe("server run registry", () => {
  it.effect("clears an accepting reservation with rollback so a later create can reserve", () =>
    Effect.gen(function* () {
  ...
  it.effect("keeps stale rollback and clear operations from clearing a different reservation", () =>
    Effect.gen(function* () {
      const registry = yield* makeServerRunRegistry();
  ...
  it.effect("keeps accepting-only rollback from clearing the same reservation after markAccepted", () =>
    Effect.gen(function* () {
      const registry = yield* makeServerRunRegistry();
  ...
  it.effect("keeps continuation clear able to release the running reservation", () =>
    Effect.gen(function* () {
      const registry = yield* makeServerRunRegistry();
      const reservation = yield* registry.reserveCreate;
