...
import { DeliveryMergeDispatchConfirmed, DeliverySnapshotDto, parseRunId } from "@gaia/core";

describe("DeliveryMergeConfirmation", () => {
  afterEach(cleanup);
  it("shows the exact destructive tuple before dispatch", async () => {
    const onConfirm = vi.fn(async () => undefined);
    render(<DeliveryMergeConfirmation actionId="merge-action-1" branch="gaia/run-1234567890" decisionSequence={42} disabled={false} headSha={"a".repeat(40)} method="squash" onConfirm={onConfirm} pending={false} prUrl="https://github.com/cill-i-am/gaia/pull/74" />);
  ...
  it("cannot open when readiness is server-disabled", () => {
    render(<DeliveryMergeConfirmation actionId="merge-action-1" branch="gaia/run-1234567890" decisionSequence={42} disabled headSha={"a".repeat(40)} method="merge" onConfirm={async () => undefined} pending={false} prUrl="https://github.com/cill-i-am/gaia/pull/74" />);
    expect(screen.getByRole("button", { name: "Merge pull request" }).hasAttribute("disabled")).toBe(true);
  });
  ...
  it("keeps a rejected confirmation open and suppresses duplicate submits", async () => {
    let reject!: () => void;
    const onConfirm = vi.fn(() => new Promise<void>((_resolve, rejectPromise) => { reject = () => rejectPromise(new Error("conflict")); }));
    render(<DeliveryMergeConfirmation actionId="merge-action-1" branch="gaia/run-1234567890" decisionSequence={42} disabled={false} error="Readiness changed; evaluate again." headSha={"a".repeat(40)} method="merge" onConfirm={onConfirm} pending={false} prUrl="https://github.com/cill-i-am/gaia/pull/74" />);
    fireEvent.click(screen.getByRole("button", { name: "Merge pull request" }));
    const confirm = await screen.findByRole("button", { name: "Confirm merge merge" });
    fireEvent.click(confirm);
    fireEvent.click(confirm);
    expect(onConfirm).toHaveBeenCalledOnce();
    reject();
    expect((await screen.findByRole("alert")).textContent).toContain("Readiness changed; evaluate again.");
